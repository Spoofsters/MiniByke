...
static struct ISR_TO_FUNC{ // I created this struc that way I can access the buffer_info variable from the twi_message_t data type. It's also static that way the buffer info remains secure.
...
const uint32_t F_CPU = 16000000; // Hard coding the F_CPU variable since for some reason VSC can't read the make file
volatile uint8_t *END_BUFFER; // Making a variable representing the last message in the buffer
bool RUFINISHED = false; // Making a variable used to see if 

twi_status_t twi_enqueue(twi_message_t *messages, size_t count){ // Adding information to the messages buffer whenever the user wants to add something. In this case it'd be firing the gun with the intent to create a bullet hole. 

  if(!(TWCR & (1<<TWINT))){ // While TWINT = 0, in other words, no data is being transmitted, run this while loop. The moment TWINT becomes 1, this becomes false and data is being transmitted.
...
twi_status_t twi_status(void){
  if((TWCR & (1<<TWINT)) && RUFINISHED == true){ // Checks if the enqueue function is not runing
    return(twi_status_success); 
  }
}
...
twi_status_t twi_deinit(void){
  if((TWCR & (1<<TWINT)) && RUFINISHED == false){ // If enqueue is running when it shouldn't then run this
    TWCR &= ~((1 << TWIE) | (1 << TWEA) | (1 << TWEN)); // Makes all of them Zero
  } 
}

static bool scl_parameters(const uint32_t set_frequency, uint8_t * const twbr, uint8_t * const twps){ // Part 4
    ...
    for(uint8_t i = 0; i < 4; i++){ // Looping between values of i going through 0, 1, 2, and 3 that are representative of 4 iterations
      ...
      twbr_checker = ((F_CPU/set_frequency) -16) / (2 * prescaler); // The behind the scene calculation that solves for the bit rate given all other variables.
        ...
        if (twbr_checker <= 255){ // Checks if the calculated bit rate can fit within the 8-bit twbr variable
...
ISR(TWI_vect){ // Using the interupt service routine(ISR) to call an interupt using cases from the Master/Slave calls  

    switch (TW_STATUS){ // A giant switch case using all of the cases the TWSR in my driver will ever use 
        case TW_START:{
        ...
        case TW_MT_DATA_ACK:{
          if (BUFFER == END_BUFFER){ // Checks if the message is at it's last element
        ...
        case TW_MR_SLA_ACK:{
            if (BUFFER < END_BUFFER){ // Check if the current buffer byte position is before end of message
...
twi_status_t twi_init(const uint32_t set_frequency){ // Creating an initializing function with the twi_status_t data type. All of the code in this function will run immediately, essentially loading the amunition and priming the trigger
  scl_parameters(100000, &TWBR, &TWSR); // Calling the scl_parameters function and passing in a hardcoded 100 KHZ frequency as well as the addresses of both TWBR and TWSR so that the pointer variables in the aforemention function work as intended.
